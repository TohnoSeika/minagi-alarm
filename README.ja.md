...
- 🖼️ **クロックモード** —— コンパクトで洗練された時計表示。ドラッグ＆リサイズ可能。最大5枚の付箋を追加して内容を自由にカスタマイズ。ミュージックプレイヤー内蔵、インポートした音楽の再生とシャッフル再生に対応
...
- 🚀 **パフォーマンス** —— インストーラ約20MB、Tauri v2 + Rust ベース、瞬時起動
...
| 📝 **付箋** | クロックモードで空白領域を右クリックして付箋を追加（最大5枚）。ドラッグで位置調整 / ダブルクリックで編集 / ピンを右クリックで削除またはMarkdownエクスポート |
...
> 💡 現在のバージョン **v1.5.0**、インストーラ約20MB。Windows版はNSISセットアッププログラムです。
...
### v1.5.0
1. ソフトウェアを正式にオープンソース化（MIT License）。
2. 潜在的なバグを修正。
